...
namespace tempogram {
    ...
    stft(const vec &s, int sr, const vec &window, std::tuple<int, int> coefficient_range, int n_fft = -1, int hop_length = -1);

    std::tuple<mat, float, vec, vec>
    stft(const vec &s, int sr, const vec &window, int n_fft = -1, int hop_length = -1);

    vec audio_to_novelty_curve(const vec &s, int sr, int window_length = -1, int hop_length = -1,
